...
const InvestimentosMicroFrontend = () => {
  const wrapperRef = useRef<HTMLDivElement>(null);
  const isMounted = useRef(false);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [InvestimentosComponent, setInvestimentosComponent] = useState<React.ComponentType<any> | null>(null);
  ...
  const loadAndMount = useCallback(async () => {
    ...
    try {
      ...
      const remoteModule = await import('funcionalidadesRemote/InvestimentosApp');
      ...
      setInvestimentosComponent(() => Component);
      setIsLoading(false);
...
export default InvestimentosMicroFrontend;
